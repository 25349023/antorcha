...
if __name__ == '__main__':
    ...
    cp = CNNParams(
        ...
        kernels=[3, 3, 3, 3],
        strides=[2, 2, 2, 1],
        bad_setting=BADSettings(batchnorm=False, activation=nn.LeakyReLU),
    )

    params = WGANGPParams(
        gen_params=gp, crtc_params=cp,
        gen_learning_rate=3e-4, crtc_learning_rate=3e-4,
        gp_weight=10,
        n_critic=5
    )
    ...
    # torch.save(wgan_gp, 'wgan_gp_mnist.pth')
    # torch.save(wgan_gp.state_dict(), 'wgan_gp_mnist_weights.pth')
